...
contract AuditTests is Test {
    ...
    function testEmergencyPauseAccessControl() public {
    ...
    function testReentrancyProtectionCommitReveal() public {
        ...
        // Mock Reality.eth finalization and result
        vm.mockCall(
            address(realityETH),
    ...
    function testMultipleIncentivesAllowed() public {
        ...
        // Create second incentive for same target - should be allowed and add to pool
        vm.prank(user2);
        bytes32 incentiveId2 = kaisign.createIncentive{value: 2 ether}(
            target, 1, 2 ether, 7 days, "second incentive"
        );
        
        // Verify pool amount increased
    ...
    function testPlatformFeeManipulation() public {
        ...
        (,,,,,address creator, address targetContract, bytes32 blobHashFromSpec, bytes32 questionId, bytes32 specIncentiveId, uint256 specChainId) = kaisign.specs(specId);
        // Mock Reality.eth to return accepted
        vm.mockCall(
            address(realityETH),
            abi.encodeWithSelector(RealityETH_v3_0.isFinalized.selector, questionId),
            abi.encode(true)
    ...
    function testBlobHashValidation() public {
        // Test empty blob hash
        bytes32 blobHash = bytes32(0);
        uint256 nonce = 12345;
        bytes32 commitment = keccak256(abi.encodePacked(blobHash, nonce));
    ...
    function testGasLimitDoS() public {
        // Start from a reasonable timestamp
        vm.warp(1000000);
        
        // Test creating many specs for same contract
        bytes32[] memory specIds = new bytes32[](10);
        bytes32[] memory commitmentIds = new bytes32[](10);
        bytes32[] memory blobHashes = new bytes32[](10);
        uint256[] memory nonces = new uint256[](10);
        uint64[] memory commitTimes = new uint64[](10);
        
        // First, commit all specs
        for (uint i = 0; i < 10; i++) {
            blobHashes[i] = keccak256(abi.encodePacked("blob", i));
            nonces[i] = i;
            bytes32 commitment = keccak256(abi.encodePacked(blobHashes[i], nonces[i]));
            
            vm.prank(user1);
            kaisign.commitSpec(commitment, target, 1);
            
            commitTimes[i] = uint64(block.timestamp);
            commitmentIds[i] = keccak256(abi.encodePacked(
                commitment, user1, target, uint256(1), commitTimes[i]
            ));
            
            vm.warp(block.timestamp + 1);
        }
        
        // Wait for all commits to be revealable
        vm.warp(block.timestamp + 30 minutes);
        ...
        bytes32[] memory retrievedSpecs = kaisign.getSpecsByContract(target, 1);
        assertEq(retrievedSpecs.length, 10);
    }
    
    function testPaginatedQueryDoS() public {
        // Start from a reasonable timestamp
        vm.warp(1000000);
        
        // Create multiple specs
        bytes32[] memory commitmentIds = new bytes32[](5);
        bytes32[] memory blobHashes = new bytes32[](5);
        uint256[] memory nonces = new uint256[](5);
        uint64[] memory commitTimes = new uint64[](5);
        ...
        // Test pagination
        (bytes32[] memory page1, uint256 total) = kaisign.getSpecsByContractPaginated(target, 1, 0, 2);
        assertEq(page1.length, 2);
        assertEq(total, 5);
        
        (bytes32[] memory page2, ) = kaisign.getSpecsByContractPaginated(target, 1, 2, 2);
        assertEq(page2.length, 2);
        
        (bytes32[] memory page3, ) = kaisign.getSpecsByContractPaginated(target, 1, 4, 2);
...
contract ReentrancyAttacker {
    ...
    function attemptReentrancyOnReveal(bytes32 commitment, bytes32 blobHash, uint256 nonce) external {
        uint64 commitTime = uint64(block.timestamp - 30 minutes);
        bytes32 commitmentId = keccak256(abi.encodePacked(
